...
import logging
from weatherdatascraper.items import GeneralWeatherItem
import re, pytz
from datetime import datetime


class HKORegionalWeatherSpider(scrapy.Spider):
    name = "HKORegionalWeather"
    ...
    def parse(self, response):
        ...
        for i in re.split('\n',report[0].extract()):
            ...
            for id, e_name, c_name, lat, lon, height in self.stations:
                if re.sub(' ','',i[:6]) == c_name:
                    ...
                    try:
                        ...
                        station['provider'] = self.PROVIDER
                    ...
                    stations[last_station]['provider'] = self.PROVIDER
                    ...
                    stations[last_station]['sea_level_height'] = height
            ...
            if len(data) > 5:
                ...
                try:
                    stations[last_station]['sea_level_pressure'] = float(data[1])
                except ValueError:
                    stations[last_station]['sea_level_pressure'] = data[1]
            elif len(data) == 2 and last_station == 'wlp':
                # Special handling to Wetland Park sea-level pressure.
                stations[last_station]['sea_level_pressure'] = data[0]


        for key in stations:
            station_item = GeneralWeatherItem()
